...
final class ColorPicker extends Gui {
    ...
    private final GuiTextField[] channels = new GuiTextField[4];
    ...
    void open(Minecraft mc, int screenW, int screenH, int rgba) {
        ...
        hexField = new GuiTextField(mc.fontRenderer, rightX + 12, hexFieldY, FIELD_W, FIELD_H);
        hexField.setMaxStringLength(9);
        for (int i = 0; i < 4; i++) {
            channels[i] = new GuiTextField(mc.fontRenderer, rightX + 12, chanY0 + i * FIELD_PITCH, FIELD_W, FIELD_H);
    ...
    void drawScreen(Minecraft mc, int mouseX, int mouseY) {
        ...
        drawRect(0, 0, overlayW, overlayH, 0x66000000);
        // 面板（不透明底，避免透出底层界面干扰取色）
        drawRect(x1, y1, x2, y2, argb(COLOR_MODAL_BG));
        drawRect(x1, y1, x2, y1 + 1, argb(COLOR_BORDER));
        drawRect(x1, y2 - 1, x2, y2, argb(COLOR_BORDER));
        drawRect(x1, y1, x1 + 1, y2, argb(COLOR_BORDER));
        drawRect(x2 - 1, y1, x2, y2, argb(COLOR_BORDER));
        ...
        okBtn.drawButton(mc, mouseX, mouseY);
    ...
    private void drawPlane() {
        ...
        setVertex(tess, 0);            // 左下黑
        tess.addVertex(planeX1, planeY2, 0.0D);
        setVertex(tess, 0);            // 右下黑
        tess.addVertex(planeX2, planeY2, 0.0D);
        setVertex(tess, pure);         // 右上纯色相
        tess.addVertex(planeX2, planeY1, 0.0D);
        setVertex(tess, 0xFFFFFF);     // 左上白
        tess.addVertex(planeX1, planeY1, 0.0D);
        tess.draw();
    ...
    private void drawThumb(int sx, int ty) {
        ...
        int y1 = cy - hh;
        int y2 = cy + hh + 1;
        drawRect(x1, y1, x2, y1 + 1, 0xFFFFFFFF);       // 上边
        drawRect(x1, y2 - 1, x2, y2, 0xFFFFFFFF);       // 下边
        drawRect(x1, y1, x1 + 1, y2, 0xFFFFFFFF);       // 左边
        drawRect(x2 - 1, y1, x2, y2, 0xFFFFFFFF);       // 右边
    }
    ...
    private static void drawCrosshair(int cx, int cy) {
        int len = 7;
        int gap = 2;
        drawRect(cx - len, cy, cx - gap, cy + 1, 0xFFFFFFFF);      // 左
        drawRect(cx + gap, cy, cx + len + 1, cy + 1, 0xFFFFFFFF);  // 右
        drawRect(cx, cy - len, cx + 1, cy - gap, 0xFFFFFFFF);      // 上
        drawRect(cx, cy + gap, cx + 1, cy + len + 1, 0xFFFFFFFF);  // 下
    }
    ...
    private void drawSwatch() {
        ...
        drawRect(rightX + 12, swatchY, rightX + 12 + FIELD_W, swatchY + SWATCH_H, argb(color));
    ...
    private void border(int bx1, int by1, int bx2, int by2) {
        ...
        drawRect(bx1, by1, bx2, by1 + 1, line);
        drawRect(bx1, by2 - 1, bx2, by2, line);
        drawRect(bx1, by1, bx1 + 1, by2, line);
        drawRect(bx2 - 1, by1, bx2, by2, line);
    }
    ...
    private static void drawVertGradient(int gx1, int gy1, int gx2, int gy2, int[] stops) {
        ...
        for (int i = 0; i < stops.length - 1; i++) {
            ...
            setVertex(tess, stops[i + 1]);
            tess.addVertex(gx1, yBot, 0.0D);
            tess.addVertex(gx2, yBot, 0.0D);
            setVertex(tess, stops[i]);
            tess.addVertex(gx2, yTop, 0.0D);
            tess.addVertex(gx1, yTop, 0.0D);
        }
        tess.draw();
        GL11.glEnable(GL11.GL_TEXTURE_2D);
        GL11.glDisable(GL11.GL_BLEND);
    ...
    private static void drawAlphaGradient(int gx1, int gy1, int gx2, int gy2, int rgb) {
        ...
        tess.setColorRGBA(r, g, b, 0);      // 底行全透明
        tess.addVertex(gx1, gy2, 0.0D);
        tess.addVertex(gx2, gy2, 0.0D);
        tess.setColorRGBA(r, g, b, 255);    // 顶行不透明
        tess.addVertex(gx2, gy1, 0.0D);
        tess.addVertex(gx1, gy1, 0.0D);
        tess.draw();
        GL11.glEnable(GL11.GL_TEXTURE_2D);
        GL11.glDisable(GL11.GL_BLEND);
        GL11.glShadeModel(GL11.GL_FLAT);
    ...
    private static void setVertex(Tessellator tess, int rgb) {
        tess.setColorRGBA((rgb >>> 16) & 255, (rgb >>> 8) & 255, rgb & 255, 255);
    }
    ...
    private void afterFieldEdit(GuiTextField field) {
        if (field == hexField) {
            applyHexText();
        } else {
            for (int i = 0; i < 4; i++) {
    ...
    private void applyChannelText(GuiTextField field, int shift) {
    ...
    private void syncFields(GuiTextField skip) {
        if (skip != hexField) {
    ...
    private GuiTextField[] allFields() {
        GuiTextField[] all = new GuiTextField[5];
        all[0] = hexField;
        System.arraycopy(channels, 0, all, 1, 4);
        return all;
    }

    private static boolean hit(GuiButton button, int mx, int my) {
        return mx >= button.xPosition && mx < button.xPosition + button.width
                && my >= button.yPosition && my < button.yPosition + button.height;
    }

    private static boolean overField(GuiTextField field, int mx, int my) {
        return mx >= field.xPosition && mx < field.xPosition + field.width
                && my >= field.yPosition && my < field.yPosition + field.height;
    }
