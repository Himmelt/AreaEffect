...
class ColorButton extends GuiButton {
    ...
    public void drawButton(Minecraft mc, int mouseX, int mouseY) {
        ...
        ColorUtil.drawCheckerboard(xPosition + 1, yPosition + 1, xPosition + width - 1, yPosition + height - 1);
        drawRect(xPosition + 1, yPosition + 1, xPosition + width - 1, yPosition + height - 1, argb(rgba));
        // 描边：悬停黄（与 FlatButton 的悬停高亮一致）
        int border = hover ? COLOR_SLIDER_THUMB_HOT : COLOR_BORDER;
        drawRect(xPosition, yPosition, xPosition + width, yPosition + 1, argb(border));
        drawRect(xPosition, yPosition + height - 1, xPosition + width, yPosition + height, argb(border));
        drawRect(xPosition, yPosition, xPosition + 1, yPosition + height, argb(border));
        drawRect(xPosition + width - 1, yPosition, xPosition + width, yPosition + height, argb(border));
