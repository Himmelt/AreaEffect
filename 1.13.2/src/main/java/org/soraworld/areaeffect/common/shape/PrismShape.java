...
public class PrismShape extends AreaShape {
    ...
    private static void writePolygonNbt(NBTTagCompound tag, double[] xs, double[] zs) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            c.putDouble("x", xs[i]);
            c.putDouble("z", zs[i]);
            // 1.13+ NBTTagList.appendTag 改名为 add
            verts.add(c);
        }
        // 1.13+ setTag 改名为 put
        tag.put("verts", verts);
    ...
    private static double[][] readPolygonNbt(NBTTagCompound tag) {
        // 1.13+ getTagList 改名为 getList（元素类型 10 = NBTTagCompound）
        NBTTagList list = tag.getList("verts", 10);
        ...
        for (int i = 0; i < count; i++) {
            NBTTagCompound c = list.getCompound(i);
            xs[i] = c.getDouble("x");
            zs[i] = c.getDouble("z");
        }
