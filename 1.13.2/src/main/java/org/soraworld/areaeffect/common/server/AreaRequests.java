...
public class AreaRequests {
    ...
    public void teleportTo(EntityPlayerMP player, int id) {
        ...
        if (player.dimension.getId() != located.dim) {
            ...
            // 1.13 跨维度转移统一走 Forge 扩展的 Entity#changeDimension(dim, ITeleporter)：
            // 传一个不建门的 teleporter，就不会在目标维度就地造/找传送门，落点随后由 center() 精确设定。
            // （1.12 时代要自己调 PlayerList#transferPlayerToDimension 并补末地离开的 placing 分支，
            //   1.13 的 changeDimension 已把整段转移补完，不再需要按来源维度打补丁。）
            player.changeDimension(target, NoPortalTeleporter.INSTANCE);
        }
    ...
    /**
     * 空实现传送门放置的 {@link ITeleporter}：只借用跨维度转移的簿记，不希望 vanilla
     * 在目标维度就地造门（自定义维度上尤其多余）。最终落点由 {@code Area#center} 精确设定。
     * 1.13 的放置钩子就是 {@link #placeEntity}（1.12 的 {@code Teleporter} 子类退化为纯实现接口）。
     */
    private static final class NoPortalTeleporter implements ITeleporter {

        static final NoPortalTeleporter INSTANCE = new NoPortalTeleporter();
