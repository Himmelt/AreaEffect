 ...
 *
 * <p>生命周期两条初始化路径：构造期只备数据（此时尚无控件、{@code fontRenderer} 也未就绪，
 * 故 {@link #reloadDims(int)} 里的 tab 实测会自行跳过），{@link #initGui()} 建控件后再
 * {@link #syncDetailWidgets()} 完成首帧同步；二者顺序是隐式契约，勿颠倒。
 *
 * <p><b>1.13 的框架差异</b>（逐处落在下面的注释里）：
 * <ul>
 *   <li>{@code buttonList} → {@code buttons}，且控件必须经 {@code addButton} 同时进
 *       {@code buttons}(绘制) 与 {@code children}(事件分发)，因此 {@link #initGui()} 里两张表一起清、
 *       统一走 {@link #addWidget}；</li>
 *   <li>{@code GuiScreen#actionPerformed} 被移除：控件自己在按下时回调宿主（见 {@link AefButton}），
 *       宿主仍按 id 分发（{@link #actionPerformed(GuiButton)}）；</li>
 *   <li>鼠标/键盘入口改名并改用浮点坐标：{@code drawScreen→render}、{@code updateScreen→tick}、
 *       {@code mouseClicked/mouseClickMove/mouseReleased} 收 double，{@code keyTyped} 拆成
 *       {@code keyPressed + charTyped}，滚轮从自取 {@code Mouse} 状态改为重写 {@code mouseScrolled}；</li>
 *   <li>{@code ScaledResolution} 移除：裁剪所需的缩放比与帧高改取 {@code Minecraft#mainWindow}。</li>
 * </ul>
 */
public class GuiAreas extends GuiScreen {
    ...
    private void layoutAddMenu() {
        int w = 20;
        for (String typeId : EffectTypes.ALL) {
            w = Math.max(w, fontRenderer.getStringWidth(translate("gui.areaeffect.effect." + typeId)) + 24);
        }
    ...
    private void drawAddMenu(int mouseX, int mouseY) {
        ...
        for (int i = 0; i < EffectTypes.ALL.size(); i++) {
            ...
            drawCenteredString(fontRenderer, translate("gui.areaeffect.effect." + typeId),
     ...
     * {@code fontRenderer} 未就绪时（构造期）直接跳过，等 {@link #initGui()} 再算。
     */
    private void measureTabs() {
        ...
        for (Integer dim : dims) {
            int w = fontRenderer.getStringWidth(tabLabel(dim)) + TAB_PAD * 2;
            tabW.add(w);
            tabTotalW += w;
        }
    }
    ...
    public void initGui() {
        ...
        // 1.13：控件要进两张表 —— buttons（绘制）与 children（事件分发）；窗口缩放会重跑 initGui，
        ...
        remarkField = new GuiTextField(0, fontRenderer, remarkX1, actY1 + (ACTION_H - 1 - 16) / 2,
    ...
    /**
     * 建控件并接线：加入绘制/事件两张表，同时把"按下"回调指回 {@link #actionPerformed(GuiButton)}。
     *
     * <p>1.13 的 {@code GuiScreen#actionPerformed} 已移除，控件不再自动通知宿主（见 {@link AefButton}），
    ...
    /** 1.13 的绘制钩子是 {@code render}（取代 {@code drawScreen}）。 */
    @Override
    public void render(int mouseX, int mouseY, float partialTicks) {
        ...
        drawBackground();
        drawCenteredString(fontRenderer, translate("gui.areaeffect.manager.title"), width / 2, 8, COLOR_TEXT_HEAD);
    ...
    private void drawTabs(int mouseX, int mouseY) {
        ...
        for (int i = 0; i < dims.size() && i < tabW.size(); i++) {
            ...
            if (x2 > tabViewX1 && x < tabViewX2) {
                ...
                drawCenteredString(fontRenderer, tabLabel(dims.get(i)), (x + x2) / 2, tabY1 + 5,
    ...
    private void drawAreaList(int mouseX, int mouseY) {
        if (areas.isEmpty()) {
            drawCenteredString(fontRenderer, translate("gui.areaeffect.manager.empty"),
        ...
        for (int i = 0; i < areas.size(); i++) {
            ...
            String id = "#" + area.id;
            int idX = areaX1 + 6 + idW - fontRenderer.getStringWidth(id);
            fontRenderer.drawStringWithShadow(id, idX, y + 5, sel ? COLOR_TEXT_HEAD : COLOR_TEXT_BODY);
            ...
            if (!remark.isEmpty()) {
                ...
                if (!shown.isEmpty()) {
                    fontRenderer.drawStringWithShadow(shown, remarkX, y + 5, COLOR_ACCENT);
                }
            }
        }
    }
    ...
    private int idColumnWidth() {
        int w = 0;
        for (Area area : areas) {
            w = Math.max(w, fontRenderer.getStringWidth("#" + area.id));
        }
        return w + 1;
    }
    ...
    private void drawEffectList(int mouseX, int mouseY) {
        ...
        for (int i = 0; i < pendingEffects.size(); i++) {
            ...
            fontRenderer.drawStringWithShadow(name, effectX1 + 6, y + 5, sel ? COLOR_TEXT_HEAD : COLOR_TEXT_BODY);
            String weight = "W" + fmt(effect.getWeight());
            fontRenderer.drawStringWithShadow(weight,
                    effectX2 - fontRenderer.getStringWidth(weight) - (ScrollColumn.BAR_W + 10),
                    y + 5, COLOR_ACCENT);
        }
        if (pendingEffects.isEmpty() && selected != null) {
            drawCenteredString(fontRenderer, translate("gui.areaeffect.manager.noeffect"),
    ...
    private void drawDetail() {
        if (selected == null) {
            drawCenteredString(fontRenderer, translate("gui.areaeffect.manager.noselect"),
    ...
    private void drawActionBar(int mouseX, int mouseY) {
        ...
        if (!shownCid.isEmpty()) {
            fontRenderer.drawStringWithShadow(shownCid, cidX1, cidY, COLOR_TEXT_HEAD);
        }
        // 备注框由 vanilla GuiTextField 自己画（保留原版观感：黑底 + 灰描边）
        remarkField.drawTextField(mouseX, mouseY, 0.0F);
    ...
    private void enableClip(int x1, int y1, int x2, int y2) {
        int s = (int) mc.mainWindow.getGuiScaleFactor();
        int fbHeight = mc.mainWindow.getHeight();
    ...
    public boolean mouseScrolled(double amount) {
        ...
        int x = (int) mc.mouseHelper.getMouseX();
        int y = (int) mc.mouseHelper.getMouseY();
    ...
    private String trim(String text, int avail) {
        ...
        String shown = fontRenderer.trimStringToWidth(text, avail);
        if (!shown.equals(text)) {
            shown = fontRenderer.trimStringToWidth(text, avail - 5) + "..";
        }
        return shown;
    }
