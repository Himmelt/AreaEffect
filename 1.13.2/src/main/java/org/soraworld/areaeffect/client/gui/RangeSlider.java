...
class RangeSlider extends AefButton {
    ...
    public void render(int mouseX, int mouseY, float partialTicks) {
        ...
        if (timed) {
            if (start <= end) {
                drawRect(scx, y + 3, ecx, y + height - 3, argb(COLOR_SLIDER_FILL));
            } else {
                // 跨午夜：填首尾两段
                drawRect(scx, y + 3, rightEdge, y + height - 3, argb(COLOR_SLIDER_FILL));
                drawRect(leftEdge, y + 3, ecx, y + height - 3, argb(COLOR_SLIDER_FILL));
            }
        } else {
            // 始终开启：整条铺满 = 全天生效，无游标
            drawRect(leftEdge, y + 3, rightEdge, y + height - 3, argb(COLOR_SLIDER_FILL));
        }
        // 悬停：边框以内整片刷一层按钮式半透明底（同时覆盖背景与进度条），再画游标使其保持清晰
        if (over) {
            drawRect(x + 1, y + 1, x + width - 1, y + height - 1, argb(COLOR_BTN_HOVER));
        }
        if (timed) {
