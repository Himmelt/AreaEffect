...
final class ColorPicker extends Gui {
    ...
    void open(Minecraft mc, int screenW, int screenH, int rgba) {
        ...
        hexField = new GuiTextField(0, mc.fontRenderer, rightX + 12, hexFieldY, FIELD_W, FIELD_H);
        hexField.setMaxStringLength(9);
        for (int i = 0; i < 4; i++) {
            channels[i] = new GuiTextField(0, mc.fontRenderer, rightX + 12, chanY0 + i * FIELD_PITCH, FIELD_W, FIELD_H);
    ...
    private static boolean overField(GuiTextField field, int mx, int my) {
        return mx >= field.x && mx < field.x + field.width
