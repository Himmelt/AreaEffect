...
public class AreaRequests {
    ...
    private static boolean changeDimension(ServerPlayerEntity player, DimensionType to,
        ...
        // 1.15.2 的 SRespawnPacket 构造器在 DimensionType 之后多了一个长整型种子参数
        player.connection.sendPacket(new SRespawnPacket(to, toWorld.getSeed(),
                oldInfo.getGenerator(), player.interactionManager.getGameType()));
        ...
        // 1.15.2 对这些此前未映射的 SRG 方法已命名：func_217447_b→addDuringPortalTeleport、
        // func_73080_a→setWorld、func_72354_b→sendWorldInfo
