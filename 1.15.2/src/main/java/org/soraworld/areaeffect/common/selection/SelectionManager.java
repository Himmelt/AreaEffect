...
public class SelectionManager {
    ...
    public Selection of(ServerPlayerEntity player) {
        return selections.computeIfAbsent(player.getUniqueID(), uuid -> new Selection());
    }

    /** 当前选区；玩家还没开始选点时返回 null。 */
    public Selection existing(PlayerEntity player) {
        return selections.get(player.getUniqueID());
    }

    /** 左键：二点形状覆盖起点；多边形追加顶点。 */
    public void clickLeft(ServerPlayerEntity player, Vec3i pos) {
        of(player).onClickLeft(pos);
        sync(player);
    }

    /** 右键：二点形状覆盖终点；多边形撤回上一顶点。{@code pos} 为 null 表示右键空气。 */
    public void clickRight(ServerPlayerEntity player, Vec3i pos) {
        of(player).onClickRight(pos);
        sync(player);
    }

    /** 切换选区形状（重置锚点）。 */
    public void reset(ServerPlayerEntity player, String type) {
    ...
    public void clear(PlayerEntity player, boolean syncClient) {
        selections.remove(player.getUniqueID());
        if (syncClient && player instanceof ServerPlayerEntity) {
