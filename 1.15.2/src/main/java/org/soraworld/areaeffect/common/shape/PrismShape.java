...
public class PrismShape extends AreaShape {
    ...
    private static void writePolygonNbt(CompoundNBT tag, double[] xs, double[] zs) {
        ListNBT verts = new ListNBT();
        int count = Math.min(xs.length, Selection.MAX_ANCHORS);
        for (int i = 0; i < count; i++) {
            CompoundNBT c = new CompoundNBT();
            c.putDouble("x", xs[i]);
            c.putDouble("z", zs[i]);
            // 1.13+ ListNBT.appendTag 改名为 add
            verts.add(c);
        }
        // 1.13+ setTag 改名为 put
        tag.put("verts", verts);
    ...
    public static PrismShape fromNbt(String type, CompoundNBT tag) {
    ...
    private static double[][] readPolygonNbt(CompoundNBT tag) {
        // 1.13+ getTagList 改名为 getList（元素类型 10 = CompoundNBT）
        ListNBT list = tag.getList("verts", 10);
        // 1.13+ tagCount 改名为 size，getCompoundTagAt 改名为 getCompound
        int count = Math.min(list.size(), Selection.MAX_ANCHORS);
