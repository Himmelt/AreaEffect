...
class FlatSlider extends AefButton {
     ...
     * 按下：进入拖拽并按落点取值。
     *
     * <p>1.14 的按下钩子是 {@code onClick}（1.13 叫 onClick、1.12 叫 {@code mousePressed(Minecraft,int,int)}），
     * 命中判定（可用 + 可见 + 落在框内）已由 {@code AefButton#mouseClicked} 先做掉；
    ...
    /**
     * 拖动中：由 {@code Screen} 只发给"按下时命中的那个控件"（见 {@link AefButton} 类注释），
     * 因此不再需要 1.7.10/1.12 时代"每帧自驱拖拽"的补丁。
     */
    @Override
    ...
    public void renderButton(int mouseX, int mouseY, float partialTicks) {
        ...
        int tx = cx - THUMB_W / 2;
        if (tx > x + 4) {
            fill(x + 3, y + 3, cx, y + height - 3, argb(COLOR_SLIDER_FILL));
        }
        // 悬停/拖拽中滑块点亮为高亮黄（与 RangeSlider 观感一致）
        boolean over = active && mouseY >= y && mouseY < y + height;
        boolean hot = dragging || (over && Math.abs(mouseX - cx) <= GRAB);
        fill(tx, y + 2, tx + THUMB_W, y + height - 2, argb(hot ? COLOR_SLIDER_THUMB_HOT : COLOR_SLIDER_THUMB));
