...
public class SkyRenderHandler {
    ...
    private static void sampleAtmosphere() {
        ...
        // 1.14 的 World#getSkyColor 收的是 BlockPos（1.13 收实体）
        Vec3d color = mc.world.getSkyColor(view.getPosition(), 0.0F);
        if (color == null) {
            return;
        }
    ...
    static final class AreaSkyRenderer implements IRenderHandler {
        ...
        @Override
        /**
         * 1.14 的 {@link IRenderHandler} 多了一个首参：{@code WorldRenderer#ticks}（一个渲染用 tick 计数，
         * 本渲染器用不到）。其余参数与 1.13 相同。
         */
        public void render(int ticks, float partialTicks, ClientWorld world, Minecraft mcw) {
            ...
            // 1.15.2 移除了 GlStateManager#color3f，改走四分量 color4f（alpha=1）
            GlStateManager.color4f(SkyRenderHandler.red, SkyRenderHandler.green, SkyRenderHandler.blue, 1.0F);
            drawSkyDome();
         ...
         */
        private static void drawSunriseSunset(ClientWorld world, float partialTicks) {
        ...
        private static void drawSun(Minecraft mcw) {
            ...
            buffer.pos(-f, 100.0D, -f).tex(0.0F, 0.0F).endVertex();
            buffer.pos(f, 100.0D, -f).tex(1.0F, 0.0F).endVertex();
            buffer.pos(f, 100.0D, f).tex(1.0F, 1.0F).endVertex();
            buffer.pos(-f, 100.0D, f).tex(0.0F, 1.0F).endVertex();
            tessellator.draw();
        }

        private static void drawMoon(Minecraft mcw, ClientWorld world) {
            float f = 20.0F;
