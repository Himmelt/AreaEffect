...
class ColorButton extends AefButton {
    ...
    public void renderButton(int mouseX, int mouseY, float partialTicks) {
        ...
        fill(x + 1, y + 1, x + width - 1, y + height - 1, argb(rgba));
        // 描边：悬停黄（与 FlatButton 的悬停高亮一致）
        int border = hover ? COLOR_SLIDER_THUMB_HOT : COLOR_BORDER;
        fill(x, y, x + width, y + 1, argb(border));
        fill(x, y + height - 1, x + width, y + height, argb(border));
        fill(x, y, x + 1, y + height, argb(border));
        fill(x + width - 1, y, x + width, y + height, argb(border));
