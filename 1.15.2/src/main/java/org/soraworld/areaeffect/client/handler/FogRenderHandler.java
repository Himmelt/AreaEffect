...
public class FogRenderHandler {
    ...
    public void onFogDensity(EntityViewRenderEvent.FogDensity event) {
        if (active && slope > EPSILON) {
            ...
            // 1.15.2 的 GlStateManager#fogMode 参数是 int（GL_FOG_MODE 常量），不再收 FogMode 枚举；
    ...
    public void onClientTick(TickEvent.ClientTickEvent event) {
        ...
        for (int i = 0; i < dust; i++) {
            double px = mc.player.getPosX() + 20 * (rand.nextDouble() - 0.5D);
            double py = mc.player.getPosY() + 10 * (rand.nextDouble() - 0.5D);
            double pz = mc.player.getPosZ() + 20 * (rand.nextDouble() - 0.5D);
