    ...
    public void onClickBlock(PlayerInteractEvent event) {
        ...
        if (player instanceof EntityPlayerMP && proxy.hasPerm(player) && proxy.isSelectTool(stack)) {
            if (event.action == PlayerInteractEvent.Action.LEFT_CLICK_BLOCK) {
                proxy.setPos1((EntityPlayerMP) player, new Vec3i(event.pos.getX(), event.pos.getY(), event.pos.getZ()), true);
                event.setCanceled(true);
            } else if (event.action == PlayerInteractEvent.Action.RIGHT_CLICK_BLOCK) {
                proxy.setPos2((EntityPlayerMP) player, new Vec3i(event.pos.getX(), event.pos.getY(), event.pos.getZ()), true);
                event.setCanceled(true);
            }
        }
    }
