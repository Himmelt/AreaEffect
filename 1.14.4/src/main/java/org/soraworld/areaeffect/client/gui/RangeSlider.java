...
class RangeSlider extends AefButton {
    ...
    /** 按下：命中游标则进入拖拽，否则整块当模式切换（1.14 的按下钩子是 {@code onClick}，命中判定已由基类做掉）。 */
    @Override
     ...
     * 1.12.2 改为 BufferBuilder 顶点流（每顶点带 color）；1.13 起状态改写统一走
     * {@link GlStateManager}（1.14 起它移到 {@code com.mojang.blaze3d.platform}，
     * 与 {@code AbstractGui.fill} 同一套缓存，避免与 vanilla 的记账错位）。
     */
    ...
    private void drawMarker(int cx, boolean isStart, int color) {
        ...
        if (isStart) {
            ...
            // 终点：方块
            fill(cx - THUMB_W / 2, top, cx + THUMB_W / 2, bot, argb(color));
        }
    }
    ...
    public void renderButton(int mouseX, int mouseY, float partialTicks) {
        ...
        boolean over = active && mouseX >= x && mouseX < x + width
        ...
        fill(x, y, x + width, y + height, argb(COLOR_SLIDER_TRACK));
        ...
        int leftEdge = x + 3;
        int rightEdge = x + width - 3;
        // 先生效区间填充
        if (timed) {
            if (start <= end) {
                fill(scx, y + 3, ecx, y + height - 3, argb(COLOR_SLIDER_FILL));
            } else {
                // 跨午夜：填首尾两段
                fill(scx, y + 3, rightEdge, y + height - 3, argb(COLOR_SLIDER_FILL));
                fill(leftEdge, y + 3, ecx, y + height - 3, argb(COLOR_SLIDER_FILL));
            }
        } else {
            // 始终开启：整条铺满 = 全天生效，无游标
            fill(leftEdge, y + 3, rightEdge, y + height - 3, argb(COLOR_SLIDER_FILL));
        }
        // 悬停：边框以内整片刷一层按钮式半透明底（同时覆盖背景与进度条），再画游标使其保持清晰
        if (over) {
            fill(x + 1, y + 1, x + width - 1, y + height - 1, argb(COLOR_BTN_HOVER));
        }
        if (timed) {
