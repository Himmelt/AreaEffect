...
public class AreaRequests {
    ...
    public void onSelectShape(EntityPlayerMP player, MessageSelectShape packet) {
        ...
        if (ShapeTypes.isValid(packet.type)) {
            ...
            LOGGER.warn("Rejected unknown select shape '{}' from {}", packet.type, player.getName());
        }
    }
     ...
     * 1.12.2 的 Teleporter 实现了 {@code ITeleporter}，实际生效的放置钩子为 {@link #placeEntity}。
     */
